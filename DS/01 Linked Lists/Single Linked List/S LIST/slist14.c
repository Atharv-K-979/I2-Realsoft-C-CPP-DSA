#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <stdbool.h>
#include <ctype.h>
#include <time.h>
#include <limits.h>
#include "slist1.h" // Definaton of application program
int main()
{
    int opt, d, sv, rv, pos, val;
    int *ptr;
    FILE *f1;
    int i = 0;
    while (1)
    {
        printf("\n1. Add End\n2. Add Beg\n3. Display\n4. Node Counts\n5. Sum of Nodes\n6. Odd cnt Sum\n7. Even cnt Sum\n8. Max of Data\n9. Min of Data\n10. Sequential Search\n11. Replace Value\n12. Delete First node\n13. Delete Last node\n14. Delete all\n15. Insert Before Pos\n16. Insert After Pos\n17. Delete ith node\n18. Reverse the Nodes\n19. Is List ordered\n20. Sort Linked List\n21. Linked List to array\n22. Array to Linked List");
        scanf("%d", &opt);
        if (opt > 22)
        {
            break;
        }
        switch (opt)
        {
        case 3:
            display();
            break;
        case 1:
        case 2:
            printf("\nData :");
            scanf("%d", &d);
            if (opt == 1)
            {
                addEnd(d);
            }
            else
            {
                addBeg(d);
            }
            break;
        case 4:
            printf("%d", methodCount());
            break;
        case 5:
            printf("%d", methodSum());
            break;
        case 6:
            printf("%d", methodOddCnt());
            break;
        case 7:
            printf("%d", methodEvenCnt());
            break;
        case 8:
            printf("%d", maxData());
            break;
        case 9:
            printf("%d", minData());
            break;
        case 10:
            printf("Enter the Search value :");
            scanf("%d", &sv);
            if (seqSearch(sv))
            {
                printf("Found !!");
            }
            else
            {
                printf("Not Found.");
            }
            break;
        case 11:
            printf("Enter the Search value :");
            scanf("%d", &sv);
            printf("Enter the replace value :");
            scanf("%d", &rv);
            findReplace(sv, rv);
            printf("Work Done !!");
            break;
        case 12:
            deleteFirstNode();
            break;
        case 13:
            deleteLastNode();
        case 14:
            deleteAll();
            break;
        case 15:
            printf("Enter Node position");
            scanf("%d", &pos);
            deliNODE(pos);
            break;
        case 16:
            printf("Enter Node position");
            scanf("%d", &pos);
            printf("Enter Node value");
            scanf("%d", &val);
            insertB(pos, val);
            break;
        case 17:
            printf("Enter Node position");
            scanf("%d", &pos);
            printf("Enter Node value");
            scanf("%d", &val);
            insertA(pos, val);
            break;
        case 18:
            reverse();
            break;
        case 19:
            printf("%d",isOrdered());
            break;
        case 20:
            sortList();
            break;
        case 21:
            d = methodCount();
            if (d != 0)
            {
                ptr = toArray(d);
                printf("\nData\n");
                for (int i = 0; i < d; i++)
                {
                    printf("%4d", ptr[i]);
                }
                free(ptr);
            }
            else
            {
                printf("\nEmpty List\n");
            }
            break;
        case 22:
            printf("Enter the cnt");
            scanf("%d",&d);
            if (d>0)
            {
                ptr=(int*)malloc(d * sizeof(int));
                for (int i = 0; i < d; i++)
                {
                    scanf("%d",(ptr+i));
                }
                deleteAll();  // removes all nodes if there are any
                toList(ptr,d);
                display();
                free(ptr);
            }
            break;
        }
    }
    return 0;
}